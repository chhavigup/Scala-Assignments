...
class Player extends Actor with TicTacToeMapper {
  override def receive = {
    case Play(PlayStep(index, player), game) => game! PlayStep(index, player)
    case TicTacToeMap(map) => printMapInArray(map)
    case  PlaceAlreadyFilled => printf("this place is already filled")
    case GameOver => sender() ! PoisonPill
  ...
  override def receive: Receive = {
    case PlayStep(index, player) => {
      if (index-1 < 0 || index-1 > 8) {
        println("you entered a invalid index")
      }
        ...
        if (map(index - 1) == 0) {
          ...
          map(index - 1) = player
          if (isGameOver(map)) {
            printf("you won!! hurry ")
            sender() ! GameOver
          }

          else sender() ! TicTacToeMap(map)
